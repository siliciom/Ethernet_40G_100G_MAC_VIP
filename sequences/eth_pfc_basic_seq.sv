//******************************************************************//
//                  ETH PFC BASIC XOFF SEQUENCE
//
// Fixed priority (PCP=3) PFC XOFF, fired probabilistically on ~10/50
// packets. Mirrors virtual_seq's basic_pfc_en branch.
//******************************************************************//
`ifndef ETH_PFC_BASIC_SEQ_SV
`define ETH_PFC_BASIC_SEQ_SV
class eth_pfc_basic_seq extends base_seq;
  `uvm_object_utils(eth_pfc_basic_seq)
  ...
  bit [2:0] pfc_prio;
  bit [15:0] pfc_quanta;
  function new(string name = "eth_pfc_basic_seq");
    super.new(name);
  endfunction
  task body();
    ...
    repeat (no_of_pkts) begin
      ...
      if (basic_pfc_en && pfc_temp && num_pkts < (no_of_pkts - 30)) begin
        ...
        // (guards against write/readback mismatch, e.g. RO bits, masking, etc.)
        ...
        req.pfc_frame_en = 0;
        //cfg_h.ral_model.tx_single_vlan_enable.write(status, 32'h1, UVM_FRONTDOOR);
        //cfg_h.ral_model.rx_single_vlan_enable.write(status, 32'h1, UVM_FRONTDOOR);
      end

...
module eth_top;
  ...
  generate
    for (gi = 0; gi < `NO_OF_AGENTS; gi++) begin : gen_config
      initial begin
        uvm_config_db#(virtual eth_interface)::set(null, $sformatf("uvm_test_top.env_h.agnt_mac[%0d]*", gi), "vif", eth_if[gi]);
      end
      ...
      for (int i = 0; i < `NO_OF_AGENTS; i++) begin
        if (!routing[i] && hdr_found[i] && !route_ready[i]) begin
          ...
          if ((hdr_base_pipe[i] + 14) <= txd_buffer[i].size()) begin
            da = {txd_buffer[i][hdr_base_pipe[i]+8],

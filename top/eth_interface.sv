...
interface eth_interface (input bit rst);
  ...
  //========================================================
     ...
     function automatic bit terminate_detected();
         for (int i = 0; i < `CTRL_WIDTH; i++) begin
             if (TXC[i] && (TXD[i*8 +: 8] == `TERMINATE_CH))
                 return 1'b1;
         end
         return 1'b0;
     endfunction
    ...
    function automatic bit rx_start_detected();
     for (int i = 0; i < `CTRL_WIDTH; i++) begin
       if ($sampled(RXC[i]) && ($sampled(RXD[i*8 +: 8]) == `START_CH))
         return 1'b1;
     end
     return 1'b0;
   endfunction  
     ...
     function automatic bit rx_terminate_detected();
         for (int i = 0; i < `CTRL_WIDTH; i++) begin
             if (RXC[i] && (RXD[i*8 +: 8] == `TERMINATE_CH))
                 return 1'b1;
         end
         return 1'b0;
     endfunction
     ...
     a_start_followed_by_terminate : assert property(p_start_followed_by_terminate)
     else
         `uvm_error("TX_START_TERM_ERR", $sformatf("Protocol Violation: Start (`START_CH) on lane 0 was either followed by an unexpected `START_CH or missing a Terminate Time=%0t", $time))
     ...
     a_rx_start_followed_by_terminate : assert property(p_rx_start_followed_by_terminate)
     else
         `uvm_error("RX_START_TERM_ERR", $sformatf("RX Protocol Violation: Start (`START_CH) on lane 0 was followed by an unexpected Start (`START_CH) or missing Terminate (`TERMINATE_CH). Time=%0t",$time));
     ...
     property p_tx_control_data_mismatch_func;
       @(posedge TX_CLK) disable iff (!rst)
       is_valid_tx_control_char();
     endproperty
     
     a_tx_control_data_mismatch_func: assert property (p_tx_control_data_mismatch_func)
     else
       `uvm_error("TX_CTRL_DATA_MISMATCH",$sformatf("Control/Data Mismatch Detected! TXC=0x%0h TXD=0x%0h Time=%0t", TXC, TXD, $time));
     ...
     property p_rx_control_data_mismatch_func;
       @(posedge RX_CLK) disable iff (!rst)
       is_valid_rx_control_char();
     endproperty
     
     a_rx_control_data_mismatch_func: assert property (p_rx_control_data_mismatch_func)
     else
       `uvm_error("RX_CTRL_DATA_MISMATCH",$sformatf("Control/Data Mismatch Detected! RXC=0x%0h RXD=0x%0h Time=%0t", RXC, RXD, $time));
     //----------------------------------------------------------------------------------
     ...
     property p_tx_terminate_followed_by_idle_all_lanes;
       ...
       is_tx_term_followed_by_idle_same_cycle()
       and
     ...
     a_tx_terminate_followed_by_idle_all_lanes : assert property(p_tx_terminate_followed_by_idle_all_lanes)
     else
       `uvm_error("TX_TERM_TO_IDLE_VIOLATION",$sformatf("Protocol Violation: TX TERMINATE character (0xFD) was NOT immediately followed by IDLE (0x07)! TXC=0x%0h TXD=0x%0h Time=%0t", TXC, TXD, $time));
     ...
     //-----------------------------------------------------------------------------------
      ...
      property p_rx_terminate_followed_by_idle_all_lanes;
       @(posedge RX_CLK) disable iff (!rst)
       // 1. Same-cycle check for Lanes 0..6
       is_rx_term_followed_by_idle_same_cycle()
       and
       // 2. Cross-cycle check for Lane 7 -> Next cycle Lane 0
      ...
      a_rx_terminate_followed_by_idle_all_lanes : assert property(p_rx_terminate_followed_by_idle_all_lanes)
      else
       `uvm_error("RX_TERM_TO_IDLE_VIOLATION",$sformatf("RX Protocol Violation: RX TERMINATE character (0xFD) was NOT immediately followed by IDLE (0x07)! RXC=0x%0h RXD=0x%0h Time=%0t", RXC, RXD, $time));
     ...
     a_tx_no_xz : assert property (p_tx_no_xz)
     else
       `uvm_error("TX_XZ",
         $sformatf("TXD/TXC contains X or Z at time %0t", $time));
     //------------------------------------------------------------
     ...
     a_rx_no_xz : assert property (p_rx_no_xz)
     else
       `uvm_error("RX_XZ",
         $sformatf("RXD/RXC contains X or Z at time %0t", $time));
     ...
     property p_fb_lane0_only;
       @(posedge TX_CLK) disable iff (!rst)
          start_detected() |-> (TXC[0] && (TXD[7:0] ==`START_CH));
     endproperty
     
     a_fb_lane0_only:
       assert property (p_fb_lane0_only)
       else
         `uvm_error("FB_LANE_ERROR",
           $sformatf("[%0t] 0xFB detected, but not on lane 0", $time))
      ...
      // ---------------------------------------------------------------------- 
       ...
       `uvm_error("RX_FB_LANE_ERROR",
           $sformatf("Time=%0t | Start detected on invalid lane! | RXC=0x%0h RXD=0x%0h | Lane0: RXC[0]=%0b RXD[7:0]=0x%0h",
           $time, RXC, RXD, RXC[0], RXD[7:0]))
      ...
      property p_tx_start_after_idle;
         @(posedge TX_CLK) disable iff (!rst)
         (TXC[0] && (TXD[7:0] == `START_CH)) |-> 
           $past(TXC[`CTRL_WIDTH-1] && (TXD[(`CTRL_WIDTH-1)*8 +: 8] == `IDLE_CH));
      endproperty
     
       a_tx_start_after_idle : assert property(p_tx_start_after_idle)
       else
         `uvm_error("TX_START_AFTER_IDLE_ERR", 
           $sformatf("Protocol Violation: TX Start (`START_CH 0x%0h) appeared on Lane 0, but previous cycle Lane %0d was NOT IDLE (0x%0h)! Prev TXD=0x%0h Time=%0t", 
                     `START_CH, `CTRL_WIDTH-1, $past(TXD[(`CTRL_WIDTH-1)*8 +: 8]), $past(TXD),$time));
      ...
      property p_rx_start_after_idle;
          ...
          (RXC[0] && (RXD[7:0] == `START_CH)) |-> $past(RXC[`CTRL_WIDTH-1] && (RXD[(`CTRL_WIDTH-1)*8 +: 8] == `IDLE_CH));
     
      endproperty
        a_rx_start_after_idle : assert property(p_rx_start_after_idle)
        else
          `uvm_error("RX_START_AFTER_IDLE_ERR", 
            $sformatf("Protocol Violation: RX Start (`START_CH 0x%0h) appeared on Lane 0, but previous cycle Lane %0d was NOT IDLE (0x%0h)! Prev RXD=0x%0h Time=%0t", 
                      `START_CH, `CTRL_WIDTH-1, $past(RXD[(`CTRL_WIDTH-1)*8 +: 8]), $past(RXD), $time));
     		 
      function automatic bit is_preamble_sfd_valid(logic [`CTRL_WIDTH*8-1:0] data_in);
       // Lanes 1 to 6 must be 0x55 (preamble)
       for (int i = 1; i <= 6; i++) begin
         if (data_in[i*8 +: 8] != `PREAMBLE)
           return 1'b0;
       end
       // Lane 7 must be 0xD5 (SFD)
       if (data_in[63:56] != 8'hD5)
         return 1'b0;
      ...
      property p_tx_start_vector_preamble;
       @(posedge TX_CLK) disable iff (!rst)
         (TXC[0] && (TXD[7:0] == `START_CH)) |-> is_preamble_sfd_valid(TXD);
      endproperty
     
      a_tx_start_vector_preamble : assert property(p_tx_start_vector_preamble)
       else
         `uvm_error("TX_START_VEC_ERR", $sformatf("TX Preamble/SFD mismatch! Time=%0t TXD=0x%0h", $time, TXD));
     
     
      property p_rx_start_vector_preamble;
       @(posedge RX_CLK) disable iff (!rst)
         (RXC[0] && (RXD[7:0] == `START_CH)) |-> is_preamble_sfd_valid(RXD);
      endproperty
     
      a_rx_start_vector_preamble : assert property(p_rx_start_vector_preamble)
       else
         `uvm_error("RX_START_VEC_ERR", $sformatf("RX Preamble/SFD mismatch! Time=%0t RXD=0x%0h", $time, RXD));
     //------------------------------------------------------------
     // Assertion: TX signals must be LOW immediately after reset deasserts
     //------------------------------------------------------------
      property p_tx_signals_low_during_reset;
       @(posedge TX_CLK)
         $rose(!rst) |=> (TXD == {`DATA_WIDTH{1'b0}} && TXC == {`CTRL_WIDTH{1'b0}});
      endproperty
      ...
      property p_rx_signals_low_during_reset;
       @(posedge RX_CLK)
         $rose(!rst) |=> (RXD == {`DATA_WIDTH{1'b0}} && RXC == {`CTRL_WIDTH{1'b0}});
      endproperty
      ...
      a_tx_clk_freq : assert property(p_tx_clk_freq)
         ...
         `uvm_error("CLK_FREQ", $sformatf("Clock period mismatch for %0dMHz! Target: %.3fns, Measured: %.3fns at Time=%0t", 
                    `FREQ_IN_MHZ, TARGET_PERIOD_NS, ($realtime - $past($realtime)), $time));
      ...
      a_tx_duty_cycle : assert property(p_tx_duty_cycle)
         ...
         `uvm_error("DUTY", $sformatf("Duty cycle mismatch for %0dMHz! Target Half-Period: %.3fns at Time=%0t", 
                    `FREQ_IN_MHZ, TARGET_HALF_PERIOD_NS, $time));
      //------------------------------------------------------------
      ...
      a_rx_clk_freq : assert property(p_rx_clk_freq)
      else
         `uvm_error("RX_CLK_FREQ", $sformatf("RX_CLK period mismatch for %0dMHz! Target: %.3fns, Measured: %.3fns at Time=%0t", 
                    `FREQ_IN_MHZ, TARGET_PERIOD_NS, ($realtime - $past($realtime)), $time));
      ...
      property p_rx_duty_cycle;
         realtime current_time;
         @(RX_CLK) disable iff (!rst) 
             (1, current_time = $realtime) |=> 
                 (($realtime - current_time) >= (TARGET_HALF_PERIOD_NS - TOLERANCE_NS)) && 
                 (($realtime - current_time) <= (TARGET_HALF_PERIOD_NS + TOLERANCE_NS));
      endproperty
     
      a_rx_duty_cycle : assert property(p_rx_duty_cycle)
      else
         `uvm_error("RX_DUTY", $sformatf("RX_CLK duty cycle mismatch for %0dMHz! Target Half-Period: %.3fns at Time=%0t", 
                    `FREQ_IN_MHZ, TARGET_HALF_PERIOD_NS, $time)); 
      ...
      //    lane[i] = TXD[i*8 +: 8];
      // 
      //  // 9C 00 00 02 in lanes 0-3
      //  if ((TXC[0] && TXC[1] && TXC[2] && TXC[3]) &&
      //      (lane[0] == 8'h02) &&
      //      (lane[1] == 8'h00) &&
      //      (lane[2] == 8'h00) &&
      //      (lane[3] == 8'h9C))
      //    return 1;
      // 
      //  // 9C 00 00 02 in lanes 4-7
      //  if ((TXC[4] && TXC[5] && TXC[6] && TXC[7]) &&
      //      (lane[4] == 8'h02) &&
      //      (lane[5] == 8'h00) &&
      //      (lane[6] == 8'h00) &&
      //      (lane[7] == 8'h9C))
      ...
      //  @(posedge TX_CLK) disable iff (!rst)
      //    seq_9c_detected()
      //      |-> ##[1:`FAULT_PERIOD] (TXC[0] && TXD[7:0] == 8'hFB);
      //endproperty
      // 
      // 
      ...
      //      "0xFB start character did not occur after 9C_00_00_02");

//******************************************************************//
//           ETHERNET MAXIMUM SIZE FRAME TEST
//
// Defines the Ethernet maximum frame size test. This test
// verifies transmission and reception of frames containing
...
class eth_max_size_frame_test extends eth_base_test;
  ...
  task run_phase(uvm_phase phase);
    ...
    for(int i=0; i<`NO_OF_AGENTS;i++) begin
      env_h.ral_model[i].tx_frame_maxlength.write(status, 32'h5EE, UVM_FRONTDOOR);
      env_h.ral_model[i].rx_frame_maxlength.write(status, 32'h5EE, UVM_FRONTDOOR);
    end

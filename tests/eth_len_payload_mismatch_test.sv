...
class eth_len_payload_mismatch_test extends eth_base_test;
  ...
  error_cb err_cb;
  eth_len_payload_mismatch_seq mac_0;
  eth_len_payload_mismatch_seq mac_1;
  ...
  task run_phase(uvm_phase phase);
    foreach (env_h.agnt_mac[i]) begin
      env_h.agnt_mac[i].mon_h.set_report_severity_id_override(UVM_ERROR, "MON_LEN_MISMATCH",
                                                              UVM_WARNING);
      env_h.agnt_mac[i].mon_h.set_report_severity_id_override(UVM_ERROR, "TX_LEN_DATA_MISMATCH",
                                                              UVM_WARNING);
      env_h.agnt_mac[i].mon_h.set_report_severity_id_override(UVM_ERROR, "RX_LEN_DATA_MISMATCH",
                                                              UVM_WARNING);
    end

    phase.raise_objection(this);

    mac_0 = eth_len_payload_mismatch_seq::type_id::create("mac_0");
    mac_1 = eth_len_payload_mismatch_seq::type_id::create("mac_1");

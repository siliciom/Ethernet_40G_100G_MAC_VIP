...
class eth_fragment_frame_test extends eth_base_test;
  ...
  eth_fragment_frame_seq mac_0;
  eth_fragment_frame_seq mac_1;
  ...
  task run_phase(uvm_phase phase);
    ...
    foreach (env_h.agnt_mac[i]) begin
      env_h.agnt_mac[i].mon_h.set_report_severity_id_override(UVM_ERROR, "TX_FRAGMENT_CRC",
                                                              UVM_WARNING);
      env_h.agnt_mac[i].mon_h.set_report_severity_id_override(UVM_ERROR, "TX_FRAGMENT_PKT",
                                                              UVM_WARNING);
      env_h.agnt_mac[i].mon_h.set_report_severity_id_override(UVM_ERROR, "MON_PADDING_ERROR",
                                                              UVM_WARNING);
      env_h.agnt_mac[i].mon_h.set_report_severity_id_override(UVM_ERROR, "RX_FRAGMENT_PKT",
                                                              UVM_WARNING);
      env_h.agnt_mac[i].mon_h.set_report_severity_id_override(UVM_ERROR, "TX_PAD_ERROR",
                                                              UVM_WARNING);
    end
    ...
    mac_0 = eth_fragment_frame_seq::type_id::create("mac_0");
    mac_1 = eth_fragment_frame_seq::type_id::create("mac_1");

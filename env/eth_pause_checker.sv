...
class eth_pause_checker extends uvm_component;
  ...
  task process_frame_bytes(int i);
    ...
    forever begin
      ...
      forever begin
        ...
        if (byte_cnt[i] < 7) begin // Preamble
          if (lane.dat != `PREAMBLE)
            `uvm_info("PCH_PREAMBLE_ERR", $sformatf("Agent - %0d, Incorrect Preamble Received =%h", i, lane.dat), UVM_LOW)
        end
        else if (byte_cnt[i] == 7) begin // SFD
          if (lane.dat != `SFD)
            `uvm_info("PCH_SFD_ERR", $sformatf("Agent - %0d, Incorrect SFD Received =%h", i, lane.dat), UVM_LOW)
        end
        ...
        if (byte_cnt[i] == 23) begin
          ...
          `uvm_info("PCH_OPC",
             $sformatf("Agent - %0d, Received Opcode = %h, Ethertype = %h", i, opcode[i], ether_type[i]), UVM_LOW)
          ...
          if (opcode[i] == 16'h0001 && ether_type[i] == 16'h8808) begin
            ...
            `uvm_info("PCH_DA_SA",
               $sformatf("Agent - %0d, Received DA = %h, SA = %h", i, da[i], sa[i]), UVM_LOW)
      ...
      if (is_pause) begin
        if (byte_cnt[i] == 72)
          `uvm_info("PCH_RX_PAUSE",
             $sformatf("Agent - %0d, Received Pause with correct size", i), UVM_LOW)
        else
        ...
        `uvm_info("PCH_RX_PAUSE_TIME",
           $sformatf("Agent - %0d, Received Pause Quanta = %0d", i, pause_time[i]), UVM_LOW)
      end
  ...
  function automatic bit is_tx_idle(int i);
    bit idle = 1;
    for (int l = 0; l < `CTRL_WIDTH; l++) begin
      if (!(v_intf[i].TXC[l] && (v_intf[i].TXD[l*8 +: 8] == `IDLE_CH)))
        idle = 0;
    end
    return idle;
  endfunction
	  ...
	  //p_time[i]--;
        ...
        if (tx_idle) begin
          ...
          if (override_pending[i]) begin
            `uvm_error("PCH_OVERRIDE_PAUSE_VIOLATION",
               $sformatf({"Agent - %0d, PAUSE was overridden to %0d quanta (%0d clocks) but ",
                          "transmitter resumed with %0d clocks of the overridden pause still remaining, Data=%h"},
                          i, pause_time[i], pause_time[i]*CLOCKS_PER_QUANTA, p_time[i], v_intf[i].TXD))
            override_pending[i] = 0;
          end
        end
  
        if (pause_override_cnt[i] >= override_gap_clks && pause_time[i] == prev_pause_time[i]) begin
          `uvm_error("PCH_PAUSE_ERR0",
             $sformatf({"Agent - %0d, Overriding Within pause not happening, Data %h is driving in interface %0d\n",
                        "Expected clocks = %0d, Actual Clocks Completed= %0d"},
                        i, v_intf[i].TXD, i, pause_time[i]*CLOCKS_PER_QUANTA, clk[i]))
        end
        else if (pause_override_cnt[i] > 0 && !tx_idle) begin
          `uvm_error("PCH_PAUSE_ERR1",
             $sformatf({"Agent - %0d, Within pause timer expiration, Data %h is driving in interface %0d\n",
                        "Expected clocks = %0d, Actual Clocks Completed= %0d"},
                        i, v_intf[i].TXD, i, pause_time[i]*CLOCKS_PER_QUANTA, clk[i]))
        end

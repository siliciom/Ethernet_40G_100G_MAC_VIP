...
class eth_packet_tracker;
  ...
  static function string get_test_name();
    if (test_name_s == "") begin
      ...
      if (test_name_s == "") test_name_s = "default_test";
    end
    return test_name_s;
  endfunction
  ...
  static function int get_mac_index(string full_name);
    ...
    end_pos = start_pos;
    while (end_pos < full_name.len() && full_name.substr(end_pos, end_pos) != "]") end_pos++;
  ...
  static function void open_file(string mac_tag);
    ...
    if (!opened.exists(mac_tag) || !opened[mac_tag]) begin
      ...
      if (fd[mac_tag] == 0) `uvm_fatal("TRACKER", $sformatf("Unable to open %s", log_file))
  ...
  //--------------------------------------------------------------------------
  static function void print_packet(string direction, string full_name, eth_seq_item tr);
    string pkt;
    string mac_tag;
    int    mac_idx;
    string arrow;
    ...
    if (direction.substr(direction.len() - 2, direction.len() - 1) == "TX") begin
      arrow   = "==>";
      dir_tag = direction;
    end else if (direction.substr(direction.len() - 2, direction.len() - 1) == "RX") begin
      arrow   = "<==";
      dir_tag = direction;
    end else begin
      arrow   = "--";
    ...
    for (int i = 0; i < tr.payload.size(); i += 16) begin
      ...
      for (int j = 0; j < 16; j++) begin
        if (i + j < tr.payload.size()) pkt = {pkt, $sformatf("%02h ", tr.payload[i+j])};
      end
      pkt = {pkt, "\n"};
    end
    pkt = {pkt, "==============================================================\n"};
  ...
  static function void close_all();
    ...
    if (fd.first(key)) begin
      do close_file(key); while (fd.next(key));
    end
  endfunction
endclass
